...
def attack(message):
    ...
    # Check if the command has 2 arguments (target, port)
    if len(command) == 3:  # Updated to accept target and port
        target = command[1]  
        port = int(command[2])  # Convert port to integer
        time = 180  # Default time set to 300 seconds
